#!/usr/bin/env python3
"""
Backfill GHL ID into Notion for all unlinked records.
Runs as a standalone persistent script - no timeout issues.
Uses cursor pagination for GHL, semaphore for concurrent Notion writes.
"""
...
NOTION_DB_ID   = "REPLACE_WITH_YOUR_NOTION_DATABASE_ID"
...
async def fetch_ghl(client):
    ...
    while url:
        resp = await client.get(url, params=params if params else None, headers=GHL_HEADERS)
        data = resp.json()
        ...
        total += len(contacts)
        sys.stdout.write(f"\r  GHL: fetched {total} contacts..."); sys.stdout.flush()

        meta = data.get("meta", {})
        next_url = meta.get("nextPageUrl")
        if next_url and len(contacts) == 100:
            # nextPageUrl already has all query params baked in — pass params=None
            url = next_url
            params = None
        else:
            url = None
...
async def fetch_notion_missing(client):
    ...
    while True:
        ...
        resp = await client.post(
            ...
            json=payload, headers=NOTION_HEADERS
        )
        data = resp.json()
...
# ── Step 3: Write GHL IDs concurrently ───────────────────────────────────────
async def backfill(client, pages, by_phone, by_name):
    ...
    # Batch in chunks for progress reporting
    chunk_size = 50
    t0 = time.time()
    for i in range(0, len(pages), chunk_size):
        ...
        print(f"  [{done}/{len(pages)}] written={written} unmatched={unmatched} errors={errors} ({rate:.1f}/s)")

    return written, unmatched, errors

# ── Main ──────────────────────────────────────────────────────────────────────
async def main():
    print("=== GHL ID Backfill ===")
    t0 = time.time()

    async with httpx.AsyncClient(timeout=120, limits=httpx.Limits(max_connections=20)) as client:
        print("Step 1: Fetching GHL contacts...")
        ...
        if not pages:
            print("Nothing to do — all Notion records already have GHL ID.")
            return
