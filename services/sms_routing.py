...
Priority:
1. Most recent outbound call number from Close API call history
...
async def _get_last_outbound_call_number(lead_id: str) -> Optional[str]:
    """Query Close API for the most recent outbound call's local_phone.

    Returns the phone number used in the most recent outbound call to this lead,
    or None if no call history.
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if data:
                local_phone = data[0].get("local_phone")
                if local_phone:
                    logger.info(
                        "Found last outbound call number for lead %s: %s",
                        lead_id,
                        local_phone,
                    )
                    return local_phone
        ...
        logger.warning(
            "Failed to query Close call history for lead %s: %s",
            lead_id,
            exc,
        )
    return None
...
async def _match_by_area_code(prospect_phone: str) -> Optional[str]:
    ...
    if prospect_state:
        ...
        if prospect_region:
            # Find any pool number in the same region
            for entry in pool:
